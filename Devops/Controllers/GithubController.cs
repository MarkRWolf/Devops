    ...
    IMemoryCache cache) : ControllerBase
{
    ...
    public async Task<ActionResult<List<GitHubWorkflowRun>>> GetWorkflowRuns()
    {
        ...
        if (runs == null) return StatusCode(500, "Failed to retrieve GitHub workflow runs.");
        return Ok(runs);
    }
    ...
    public async Task<ActionResult<List<GitHubWorkflowRun>>> GetProjectWorkflowRuns()
    {
        ...
        var key = $"runs-{_projectOwnerRepo}";
        if (!_cache.TryGetValue(key, out List<GitHubWorkflowRun>? runs))
        {
            runs = await _githubService.GetWorkflowRunsWithPatAsync(owner, repo, _projectPat);
            if (runs != null) _cache.Set(key, runs, TimeSpan.FromSeconds(10));
        }

        if (runs == null) return StatusCode(500, "Failed to retrieve project GitHub workflow runs.");
        return Ok(runs);
    }
